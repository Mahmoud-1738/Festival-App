...
function InfoScreen({ t, th, dark }) {
  ...
  const sections = [
    ...
    { key: 'sec4', icon: 'help_outline',   title: t.sec4Title, items: t.sec4Items },
    { key: 'sec5', icon: 'stars',          title: t.sec5Title, body: t.sec5Body, accent: BRAND.saffron },
  ];
        ...
        {sections.map(sec => {
          ...
          return (
            <div key={sec.key} style={{
              borderRadius: 16, background: th.cardBg,
              border: `1px solid ${isOpen ? accent + '66' : th.border}`,
              ...
              <button onClick={() => setOpen(isOpen ? null : sec.key)}
                ...
                <div style={{ flex: 1, textAlign: 'left', fontFamily: 'Sansation', fontWeight: 700, fontSize: 15, color: th.text, letterSpacing: '-0.2px' }}>{sec.title}</div>
              ...
              <div style={{ maxHeight: isOpen ? 800 : 0, overflow: 'hidden', transition: 'max-height .35s cubic-bezier(.4,0,.2,1)' }}>
                <div style={{ padding: '0 16px 16px 62px' }}>
                  {sec.body && <div style={{ fontFamily: 'Sansation', fontSize: 13, color: th.text2, lineHeight: 1.7, whiteSpace: 'pre-line' }}>{sec.body}</div>}
                  {sec.items && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {sec.items.map((item, i) => (
                        <div key={i} style={{ paddingBottom: 10, borderBottom: i < sec.items.length - 1 ? `1px solid ${th.border}` : 'none' }}>
                          <div style={{ fontFamily: 'Sansation', fontWeight: 700, fontSize: 13, color: th.text, marginBottom: 4 }}>{item.t}</div>
                          <div style={{ fontFamily: 'Sansation', fontSize: 12, color: th.text2, lineHeight: 1.6 }}>{item.b}</div>
                        </div>
                      ))}
                    </div>
                  )}
...
Object.assign(window, { HomeScreen, InfoScreen, ScheduleScreen });
