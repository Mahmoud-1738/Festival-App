...
// Password-gated editor for the lineup. Talks to the PHP/MySQL
// API and persists changes in the database for every visitor.
// ═══════════════════════════════════════════════════════════
...
function Login({ onLogin }) {
  ...
  const submit = async (e) => {
    ...
    setBusy(true); setErr('');
    try {
      const { token } = await api('login.php', { method: 'POST', body: { password: pw }, auth: false });
...
function ActModal({ initial, day, stages, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) await api(`acts.php?id=${initial.id}`, { method: 'PUT', body: payload });
      else await api('acts.php', { method: 'POST', body: payload });
      onSaved();
  ...
  const remove = async () => {
    ...
    setBusy(true); setErr('');
    try { await api(`acts.php?id=${initial.id}`, { method: 'DELETE' }); onSaved(); }
    catch (e) { setErr(e.message); setBusy(false); }
  };
...
function Dashboard({ onLogout }) {
  ...
  const reload = useCallback(async () => {
    try { setData(await api('schedule.php', { auth: false })); }
...
function App() {
  ...
  const logout = async () => {
    // Stateless tokens — just drop it locally.
