...
function App() {
  ...
  // On phones, fill the whole screen; on desktop keep the framed mockup.
  const [isMobile, setIsMobile] = uS(typeof window !== 'undefined' && window.innerWidth <= 600);
  uE(() => {
  ...
  const frame = isMobile
    ? { width: '100vw', height: '100dvh', borderRadius: 0, boxShadow: 'none' }
    : { width: 375, height: 812, borderRadius: 40, boxShadow: dark ? '0 30px 80px rgba(0,0,0,0.9), 0 0 0 1px rgba(255,255,255,0.06)' : '0 30px 80px rgba(0,0,0,0.35), 0 0 0 1px rgba(0,0,0,0.12)' };
  ...
  return (
    <div>
      <div style={{ ...frame, background: th.bg, overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative', fontFamily: 'Sansation', transition: 'background .3s' }}>
